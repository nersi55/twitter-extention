    ...
    try {
      chrome.storage.local.set({ [storageKey]: res }, () => {
        chrome.runtime.sendMessage({ action: messageAction, result: res }, () => {
          if (chrome.runtime.lastError) console.warn(messageAction + ' - no recipient:', chrome.runtime.lastError.message);
          else console.log(messageAction + ' delivered');
        });
      });
    } catch (e) {
      console.warn('deliverResult storage set failed:', e);
      try { chrome.runtime.sendMessage({ action: messageAction, result: res }, () => {}); } catch (e2) { console.warn('deliverResult sendMessage failed:', e2); }
    }
  } else {
    // storage not available, attempt direct message
    try {
      chrome.runtime.sendMessage({ action: messageAction, result: res }, () => {
        if (chrome.runtime.lastError) console.warn(messageAction + ' - no recipient and storage unavailable:', chrome.runtime.lastError.message);
    ...
    const run = () => {
      chrome.scripting.executeScript(options, (results) => {
        if (chrome.runtime.lastError) {
          ...
          const transient = /Frame with ID .* was removed|The message port closed|Could not establish connection/i.test(msg);
          if (transient && attempt < retries) {
            attempt++;
            const delay = baseDelay * Math.pow(2, attempt - 1);
            console.warn(`executeScript transient error (attempt ${attempt}/${retries}):`, msg, '— retrying in', delay, 'ms');
          ...
          executeScriptWithRetries({
            ...
            func: async (count) => {
              ...
              const getRetweetButtons = () => {
                ...
                Array.from(document.querySelectorAll('[data-testid="retweet"], div[data-testid="retweet"]')).forEach((el) => {
                  const btn = el.closest('div[role="button"], button') || el;
                  if (btn && !candidates.some(c => c.btn === btn)) candidates.push({ el, btn, reason: 'data-testid' });
                });
              ...
              const clickButtonByText = (text) => {
                const nodes = Array.from(document.querySelectorAll('div[role="menuitem"], div[role="button"], button'));
              ...
              while (retweeted < count && scrollAttempts < maxScrolls) {
                ...
                console.log('Retweet candidates:', candidates.length, 'retweeted so far:', retweeted);
                ...
                for (const { el, btn } of candidates) {
                  ...
                  try {
                    ...
                    const dispatchMouse = (node) => {
                      try {
                        ['mouseover','mousedown','mouseup','click'].forEach(type => node.dispatchEvent(new MouseEvent(type, { bubbles: true, cancelable: true, view: window })));
                        return true;
                      } catch(e) { try { node.click(); return true; } catch(e2) { return false; } }
                    };

                    // Prefer searching inside visible menus/dialogs that pop up
                    const menus = Array.from(document.querySelectorAll('div[role="menu"], div[role="dialog"], div[role="presentation"]'));
                    const visibleMenu = menus.find(m => m && (m.offsetParent !== null || m.getClientRects().length > 0));

                    if (visibleMenu) {
                      for (const opt of tryOptions) {
                        const match = Array.from(visibleMenu.querySelectorAll('*')).find(n => n.textContent && new RegExp('\\b' + opt + '\\b', 'i').test(n.textContent.trim()));
                        if (match) {
                          if (dispatchMouse(match)) { console.log('Clicked menu option in visible menu:', opt); clickedConfirm = true; break; }
                        }
                      }
                    }
                    ...
                    if (!clickedConfirm) {
                      for (const opt of tryOptions) {
                        if (clickButtonByText(opt)) { console.log('Clicked menu option (global search):', opt); clickedConfirm = true; break; }
                      }
                    }

                    if (!clickedConfirm) {
                      // Fallback: search menu-like nodes for matching text and click
                      const menuNodes = Array.from(document.querySelectorAll('div[role="menuitem"], div[role="menu"] button, div[role="menu"] div, button'));
                      const match = menuNodes.find(n => n.textContent && /\b(Repost|Retweet|Quote|Quote Tweet)\b/i.test(n.textContent.trim()));
                      if (match) {
                        if (dispatchMouse(match)) { clickedConfirm = true; console.log('Clicked fallback menu node:', match.textContent.trim().slice(0,40)); }
                        else try { match.click(); clickedConfirm = true; console.log('Clicked fallback menu node via click():', match.textContent.trim().slice(0,40)); } catch (e) { console.warn('Fallback menu click failed', e); }
                      } else {
                        console.warn('No menu option found by text; menu nodes count:', menuNodes.length);
                      }
                    }
                    ...
                    const becameRetweeted = nowAria === 'true' || !!btn.querySelector && !!btn.querySelector('[data-testid="unretweet"]') || !btn.querySelector('[data-testid="retweet"]');
          ...
          executeScriptWithRetries({
            ...
            func: async (count, messages) => {
              ...
              const getQuoteCandidates = () => {
                ...
                Array.from(document.querySelectorAll('[data-testid="retweet"], [data-testid="like"] , div[data-testid]')).forEach((el) => {
                ...
                Array.from(document.querySelectorAll('div[role="button"] svg')).forEach((svg) => {
                  const t = svg.getAttribute('aria-label') || (svg.querySelector('title') && svg.querySelector('title').textContent) || '';
                  if (/retweet|repost/i.test(t)) {
                    const btn = svg.closest('div[role="button"], button');
                    if (btn && !candidates.some(c => c.btn === btn)) candidates.push({ el: svg, btn });
                  }
                });
              ...
              const waitForComposer = async (timeoutMs = 8000) => new Promise((resolve, reject) => {
                const start = Date.now();
                const timer = setInterval(() => {
                  const composer = document.querySelector('div[role="dialog"] [role="textbox"], div[role="textbox"][data-testid], textarea');
                  if (composer) {
                    clearInterval(timer);
                    resolve(composer);
                    return;
              ...
              const setComposerText = (el, text) => {
                ...
                } catch (e) { console.warn('setting value failed:', e); }
                // last resort: set innerText
                try { el.innerText = text; el.dispatchEvent(new Event('input', { bubbles: true })); return true; } catch (e) { return false; }
              };
              ...
              while (quoted < count && scrollAttempts < maxScrolls) {
                ...
                for (let i = 0; i < candidates.length && quoted < count; i++) {
                  ...
                  try {
                    ...
                    const quoteOptions = ['Quote', 'Quote Tweet', 'Retweet with comment', 'Add a comment'];
                    let clicked = false;
                    ...
                    if (visibleMenu) {
                      for (const o of quoteOptions) {
                        const match = Array.from(visibleMenu.querySelectorAll('*')).find(n => n.textContent && new RegExp('\\b' + o + '\\b', 'i').test(n.textContent.trim()));
                        if (match) { if (dispatchMouse(match)) { clicked = true; console.log('Clicked menu option in visible menu:', o); break; } }
                      }
                    }

                    if (!clicked) {
                      for (const o of quoteOptions) {
                        if (clickButtonByText(o)) { clicked = true; console.log('Clicked menu option (global search):', o); break; }
                      }
                    }

                    if (!clicked) {
                      // fallback: try nodes in menus
                      const menuNodes = Array.from(document.querySelectorAll('div[role="menuitem"], div[role="menu"] button, div[role="menu"] div'));
                      const match = menuNodes.find(n => n.textContent && /\b(Quote|Quote Tweet|Retweet with comment|Add a comment)\b/i.test(n.textContent.trim()));
                      if (match) { try { match.click(); clicked = true; console.log('Clicked fallback menu node'); } catch (e) { console.warn('fallback click failed', e); } }
                    }

                    if (!clicked) { console.warn('No quote option found for candidate'); continue; }

                    // Wait for composer
                    let composer;
                    try { composer = await waitForComposer(8000); } catch (e) { console.warn('Composer did not appear:', e); continue; }
                    ...
                    try {
                      ...
                      if (whoDialog) {
                        console.log('Who dialog detected (pre-click). Truncated HTML:', whoDialog.outerHTML.slice(0, 500));
                        // Prefer aria-label if present, otherwise match text content; fallback to first selectable option
                        let everyoneBtn = whoDialog.querySelector('[aria-label*="Everyone" i], [aria-label*="everyone" i]');
                        if (!everyoneBtn) {
                        ...
                        if (everyoneBtn) {
                          try { dispatchMouse(everyoneBtn); console.log('Selected Everyone for Who can reply'); } catch (e) { try { everyoneBtn.click(); console.log('Clicked Everyone fallback'); } catch (e2) { console.warn('Click Everyone fallback failed', e2); } }
                          await wait(300 + Math.random() * 300);
                        } else {
                          // fallback: click first option or radio-like candidate
                          const optionCandidates = Array.from(whoDialog.querySelectorAll('[role="menuitem"], [role="menuitemradio"], div[role="button"], button, li'))
                            .filter(n => n && n.offsetParent !== null && (n.textContent || '').trim().length > 0 && !/Who can reply|Choose who can reply/i.test(n.textContent || ''));
                          if (optionCandidates.length) {
                            const opt = optionCandidates[0];
                            try { dispatchMouse(opt); console.log('Clicked first reply option (fallback):', (opt.textContent||'').trim().slice(0,60)); } catch (e) { try { opt.click(); console.log('Clicked first reply option fallback via click:', (opt.textContent||'').trim().slice(0,60)); } catch (e2) { console.warn('Clicking first option failed', e2); } }
                            await wait(300 + Math.random() * 300);
                          } else {
                            console.warn('Everyone button not found and no option candidates in Who dialog (pre-click)');
                          }
                          ...
                          if (doneBtn) {
                            try { dispatchMouse(doneBtn); console.log('Clicked Done/Apply to close Who dialog (pre-click)'); } catch(e) { try { doneBtn.click(); console.log('Clicked Done fallback (pre-click)'); } catch(e2) { console.warn('Click Done fallback failed (pre-click)', e2); } }
                    ...
                    } catch (e) { console.warn('Error handling Who can reply dialog (pre-click):', e); }
                    ...
                    if (composerDialog) {
                      tweetBtn = Array.from(composerDialog.querySelectorAll('div[role="button"], button')).find(n => n.textContent && btnMatcher.test(n.textContent.trim()));
                    }
                    if (!tweetBtn) {
                      ...
                      tweetBtn = tweetButtons.find(b => /Tweet|Post/i.test(b.textContent)) || tweetButtons[0];
                    }
                    ...
                    if (tweetBtn) {
                      try {
                        ...
                        if (!posted) {
                          const menus = Array.from(document.querySelectorAll('div[role="dialog"], div[role="menu"]'));
                          const whoDialog = menus.find(d => /Who can reply/i.test(d.textContent || ''));
                          if (whoDialog) {
                            console.log('Who dialog detected. OuterHTML snapshot (truncated):', whoDialog.outerHTML.slice(0, 800));
                            let everyoneBtn = Array.from(whoDialog.querySelectorAll('div[role="button"], button')).find(n => /Everyone/i.test(n.textContent || ''));
                            if (everyoneBtn) {
                              try { dispatchMouse(everyoneBtn); console.log('Selected Everyone for Who can reply (auto)'); } catch (e) { try { everyoneBtn.click(); console.log('Clicked Everyone fallback'); } catch (e2) { console.warn('Click Everyone fallback failed', e2); } }
                              await wait(300 + Math.random() * 300);
                            } else {
                              ...
                              if (optionCandidates.length) {
                                ...
                                console.warn('Everyone button not found and no option candidates in Who dialog');
                              }

                              // After selecting, some UIs require clicking a 'Done' or 'Apply' button or pressing Escape
                              const doneBtn = Array.from(whoDialog.querySelectorAll('div[role="button"], button')).find(n => /Done|Apply|Close|Save|Done|OK/i.test(n.textContent || ''));
                              if (doneBtn) {
                                try { dispatchMouse(doneBtn); console.log('Clicked Done/Apply to close Who dialog'); } catch(e) { try { doneBtn.click(); console.log('Clicked Done fallback'); } catch(e2) { console.warn('Click Done fallback failed', e2); } }
                                await wait(200 + Math.random() * 300);
                              } else {
                              ...
                              const stillWho = Array.from(document.querySelectorAll('div[role="dialog"], div[role="menu"]')).find(d => /Who can reply/i.test(d.textContent || ''));
                        ...
                        if (!posted) {
                          try {
                            if (tweetBtn) {
                              try { tweetBtn.focus(); } catch (e) {}
                              try { tweetBtn.click(); } catch (e) { try { dispatchMouse(tweetBtn); } catch (e2) {} }
                            ...
                            if (!posted) {
                              try {
                                const composerDialogLocal = composer && composer.closest ? composer.closest('div[role="dialog"]') : document;

                                // 1) Direct known data-testids (fast path)
                                const knownSelectors = ['[data-testid="tweetButton"]', '[data-testid="tweet-button"]', '[data-testid="tweet_submit"]', '[data-testid="toolBar"]', '[data-testid="tweetButtonInline"]'];
                                let postBtn = null;
                                for (const sel of knownSelectors) {
                                  const found = composerDialogLocal.querySelector(sel) || document.querySelector(sel);
                                  if (found) { postBtn = found; console.log('Found post button by known selector:', sel); break; }
                                }

                                // 2) Prefer buttons with visible 'Post' or 'Tweet' text outside Who dialog
                                if (!postBtn) {
                                  const postCandidates = Array.from((composerDialogLocal || document).querySelectorAll('div[role="button"], button, [data-testid]'));
                                  const isInsideWhoDialog = (node) => {
                                    let cur = node;
                                    while (cur) {
                                      try { if (/Who can reply|conversation-controls|conversation-controls-title/i.test(cur.textContent || (cur.id||''))) return true; } catch (e) {}
                                      cur = cur.parentElement;
                                    }
                                    return false;
                                  };
                                  const filteredCandidates = postCandidates.filter(n => n && !isInsideWhoDialog(n));

                                  postBtn = filteredCandidates.find(n => n && n.textContent && /\b(Post|Tweet)\b/i.test((n.textContent||'').trim()));
                                  if (!postBtn) postBtn = filteredCandidates.find(n => n && n.getAttribute && /tweet|post|submit|send/i.test((n.getAttribute('data-testid')||'')));
                                  if (!postBtn) postBtn = filteredCandidates.find(n => n && n.getAttribute && /post|tweet|send/i.test((n.getAttribute('aria-label')||'')));
                                }

                                // 3) Last resort: find any visible button not inside Who dialog
                                if (!postBtn) {
                                  const allBtns = Array.from((composerDialogLocal || document).querySelectorAll('button, [role="button"]'));
                                  const candidate = allBtns.find(n => n && n.offsetParent !== null && !(n.closest && n.closest('[aria-labelledby="conversation-controls-title"],[aria-describedby="conversation-controls-details"]')) && (n.textContent||'').trim().length > 0);
                                  if (candidate) { postBtn = candidate; console.log('Found post button by last-resort candidate'); }
                                }

                                if (postBtn) {
                                  console.log('Attempting definitive post button click. Text:', (postBtn.textContent||'').trim().slice(0,40), 'data-testid:', postBtn.getAttribute && postBtn.getAttribute('data-testid'), 'aria-label:', postBtn.getAttribute && postBtn.getAttribute('aria-label'));
                                  try { postBtn.focus(); } catch (e) {}
                                  try { postBtn.click(); } catch (e) { try { dispatchMouse(postBtn); } catch (e2) { console.warn('Definitive post click failed', e2); } }
                                  await wait(500 + Math.random() * 500);
                                  const stillFinal = document.querySelector('div[role="dialog"] [role="textbox"], div[role="textbox"][data-testid], textarea');
                                  if (!stillFinal) { posted = true; console.log('Definitive post button succeeded'); }
                                  else { console.warn('Definitive post button click did not close composer'); console.log('Post button outerHTML (truncated):', (postBtn.outerHTML||'').slice(0,400)); }
                                } else {
                                  console.warn('No definitive post button candidate found in composer');
                                }
                              } catch (e) { console.warn('Error trying definitive post button:', e); }
                            }
                        ...
                        if (!posted) {
                          console.warn('Posting did not succeed after keyboard and click fallbacks; skipping candidate');
                          continue;
                        }
                      } catch (e) {
